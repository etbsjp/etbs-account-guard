...
- ★★ **保存時に `sanitize_textarea_field()` を通す**（1.2.1・wordpress.org 審査の指摘）。アドレスと CIDR の
  文字集合（`0-9 a-f A-F : . /`）は一切影響を受けないので、**通る許可先が変わることは無い**。ただし
  **`#` 以降のメモは加工されうる**：`<` を含むメモはタグに見える部分が落ち、パーセント符号化（`%` ＋16進2桁）
  はその部分が消える。さらに `%xx` が1つでもあると、その欄**全体**の連続する空白が1つに詰められる（桁揃えが崩れる）。
  `%xx` が無ければ桁揃えは保たれる。
- ★★ **不正な UTF-8 が1バイトでもあると `wp_check_invalid_utf8()` が欄全体を空文字にする。**
  そのまま保存すると利用者の一覧が無言で全部消えるため、サニタイズの結果が空になったら元の文字列に戻し、
  不正な行として明示的に拒否させる。実ブラウザのフォーム送信では到達しない（常に UTF-8 で送るため）。
