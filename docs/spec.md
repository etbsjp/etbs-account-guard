...
- ★ **例外：判定そのものを行わない経路の例外は記録しない。** `ACGD_Basic_Auth::maybe_strip_confirmed_header()`（`determine_current_user` の優先度15）がそれで、ここは「確認済み・保存前の BASIC 資格情報を `$_SERVER` から剥がす」だけの経路であり、誰を通すかの判定には一切関与しない。上の表の「止めて通す＋警告」は記録（`record_fault()`）を伴い、それは **`is_disabled()` = `is_switch_disabled() || has_fault()`** という機能停止スイッチを引くことを意味する。**この経路の失敗の代償**は、本体の赤い通知（「サイトでは Basic 認証が使われているようですが……」）が並ぶことと、確認から保存までの窓の間その管理者の REST が 401 になること（本体の `wp_validate_application_password()` が剥がし損ねたヘッダーを拾うため。5.3 ★★★）に留まる。**スイッチを引く代償のほうが大きい**ので引かない。
