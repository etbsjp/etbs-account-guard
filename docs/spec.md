...
| 本人のプロフィール画面（`profile.php`） | 何も出さない（許可されている場所の手がかりを与えない） | ― |

★ 「ユーザーの編集画面」の区画は本体の `edit_user_profile`（`profile.php` 以外の経路のときだけ発火。対象が
本人かどうかは関係ない）に掛けている。そのため「設定 > ETBS Account Guard」の「アクセス制限」タブに、
`get_edit_profile_url()`（自分の ID には常に `profile.php` を強制する）を経由しない、
`user-edit.php?user_id=<自分のID>` への生のリンクを置き、本人が自分自身の BASIC 認証資格情報を
設定・確認できる導線にする（5.3 の「自分をBASICモードにするときは...確認ボタンで一度確認画面を通ってから
保存する」に要る）。通常の導線（ツールバー・ユーザー一覧の自分の行）は今までどおり `profile.php` を経由する
ため、上の行（本人のプロフィール画面には何も出さない）は変わらない。
